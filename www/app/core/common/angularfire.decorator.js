	...
	angular.module("app.core")
		.config([
			"$provide", function($provide) {
				$provide.decorator("$firebaseAuth", [
					"$delegate", "$q", "$window", "firebaseAltUrl", function($delegate, $q, $window, firebaseAltUrl) {
						...
						$delegate = function(ref) {
							...
							function authAnonymously(options) {
								...
								//first time login as anonymous user
								if (!!currentUser) {
									//we need to make sure that the user we found used anon auth
									//otherwise we need to add the provider. this will be very seldom.
									//if the user used anon auth we are going to return the user and not auth again as this
									//will create a new instance in the db.
									var authGroupRef = new Firebase(ref.root() + "/authGroup/" + currentUser.authGroup);
									authGroupRef.orderByKey().equalTo("anonymous").once("value", function(snapshot) {
										var existsAuthGroup = (snapshot.val() !== null);
										if (currentUser.provider === "anonymous" || existsAuthGroup) {
											deferred.resolve(currentUser);
										} else {
											baseAuthAnon(options, deferred);
										}
									});
								} else {
									baseAuthAnon(options, deferred);
							...
							function associateSocial(provider) {
								var deferred = $q.defer();
								var newRef = new Firebase(firebaseAltUrl);
											...
											.then(function(user) {
												...
												userMappingRef.once("value", function(snapshot) {
													...
													if (exists) {
														...
														//todo test this so that the email is updated.
							...
							*/
							...
							/*
							   this will anonymously authenticate a user when logging in
						 */
							function baseAuthAnon(options, deferred) {
