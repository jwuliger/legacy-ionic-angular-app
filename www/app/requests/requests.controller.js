(function() {
	...
	RequestsController.$inject = ["$firebaseArray", "$scope", "$cordovaCamera", "fileTransfer", "cordovaToast", "fbDataRef", "userProfile", "$ionicHistory", "$state", "requestService", "geoService"];

	/* @ngInject */
	function RequestsController($firebaseArray, $scope, $cordovaCamera, fileTransfer, cordovaToast, fbDataRef, userProfile, $ionicHistory, $state, requestService, geoService) {
		...
		function addRequest() {
			geoService.getCurrentPosition(function(position) {
				if (position.coords.latitude) {					
					if (userProfile) {
						$scope.newRequest.lat = position.coords.latitude;
						$scope.newRequest.long = position.coords.longitude;
						...
						requests.$add($scope.newRequest).then(function (ref) {
							...
							if ($scope.imageSrc) {
								fileTransfer.uploadFile($scope.imageSrc, "Request Uploads", "request", id).then(function() {
						...
						console.log("RequestsController - Add New Request", "Ukown Error Occured with loading 'userProfile'");
					}
				}
			});
		}
		...
		function getPicture(source) {
			document.addEventListener("deviceready", function() {
				...
				$cordovaCamera.getPicture(options).then(function (imageUri) {
					...
					var localPath = "cdvfile://" + imageUri.substring(imageUri.lastIndexOf("/Users/"), imageUri.length);
				...
				(function() {
					...
					ref.orderByChild("categories").on("value", function(snapshot) {
						var findMatch = _.filter(
							_.map(snapshot.val(), filterCategory), function(element) {
